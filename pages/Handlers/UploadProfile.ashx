...
public class Upload : IHttpHandler
{
    ...
    public void SaveFiles(FileDetails fd)
    {
        try
        {
            ...
            var commandText = @"MERGE INTO TBL_M_USER_MASTER_ATTACHMENT AS target
                USING (VALUES (@USERID, @TYPE)) AS source (USER_ID, IMAGE_TYPE)
                ON target.USER_ID = source.USER_ID AND target.IMAGE_TYPE = source.IMAGE_TYPE
                WHEN MATCHED THEN
                    UPDATE SET target.DESCRIPTION = @DESCRIPTION, target.CREATED_DATE = GETDATE()
                WHEN NOT MATCHED THEN
                    INSERT (USER_ID, DESCRIPTION, IMAGE_TYPE, CREATED_DATE)
                    VALUES (source.USER_ID, @DESCRIPTION, source.IMAGE_TYPE, GETDATE());";
             var parameters = new
            {
                USERID = fd.UserId,
